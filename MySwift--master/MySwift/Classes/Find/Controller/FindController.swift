...
//        TTAlamoNetWorkTool.requestWith(Method: .get, URL: "https://api.douban.com/v2/movie/in_theaters", Parameter: nil, Token: nil) { (res) in
        TTAlamoNetWorkTool.requestWith(Method: .get, URL: "https://qqlykm.cn/api/hotlist/get?key=1yb9wvlVGVoCiOpSlo4E5Rgovv&type=baidu", Parameter: nil, Token: nil) { (res) in
...
//            }
    ...
    func setUpFlowLayout(flowLayout: UICollectionViewFlowLayout){
        ...
        let itemW = ((SCREENWIDTH-20) - (itemCountInRow+1)*margin) / itemCountInRow
        let itemH = itemW * 1.3
...
//        present(detail, animated: true, completion: nil)
        ...
        let webvc = WKWebVC();
        let model:HomeModel = self.dataArray?.object(at: indexPath.row) as! HomeModel
