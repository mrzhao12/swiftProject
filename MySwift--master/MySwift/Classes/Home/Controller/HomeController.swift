    ...
    func numberOfSections(in tableView: UITableView) -> Int {
     ...
     func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.btnClickBlock = {()in
            ...
            let webvc = WKWebVC();
            webvc.tturl = model.url
            self.navigationController?.pushViewController(webvc, animated: true)
            
//            let myweb = WKWebView(frame: <#T##CGRect#>)
...
//        TTAlamoNetWorkTool.requestWith(Method: .get, URL: "https://api.douban.com/v2/movie/in_theaters?apikey=0df993c66c0c636e29ecbb5344252a4a", Parameter: nil, Token: nil) { (res) in
        
//        TTAlamoNetWorkTool.requestWith(Method: .get, URL: "https://qqlykm.cn/api/joke/?key=1yb9wvlVGVoCiOpSlo4E5Rgovv", Parameter: nil, Token: nil) { (res) in
    
        TTAlamoNetWorkTool.requestWith(Method: .get, URL: "https://qqlykm.cn/api/history/get?key=1yb9wvlVGVoCiOpSlo4E5Rgovv", Parameter: nil, Token: nil) { (res) in
...
//                if(startInt==0)
//            {
                   let subArray = dic["data"] as! NSMutableArray
                    for dic in subArray{
                        //print(dic)
